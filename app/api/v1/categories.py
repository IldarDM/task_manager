...
async def list_categories(
    *,
    ...
    return CategoryService.get_user_categories(db=db, user_id=current_user.id, skip=skip, limit=limit)
...
async def create_category(
    *,
    ...
    """Create new category."""
    rate_limit_general(request)
    return CategoryService.create_category(db=db, category_create=category_in, user_id=current_user.id)
...
async def get_category(
    *,
    ...
    category = CategoryService.get_category_by_id(db=db, category_id=category_id, user_id=current_user.id)
    if not category:
...
async def update_category(
    *,
    ...
    return CategoryService.update_category(
        db=db, category_id=category_id, category_update=category_update, user_id=current_user.id
    )
...
async def delete_category(
    *,
    ...
    """Delete category."""
    rate_limit_general(request)
    CategoryService.delete_category(db=db, category_id=category_id, user_id=current_user.id)
    return SuccessResponse(message="Category deleted successfully", data={"category_id": category_id})
...
async def get_category_tasks(
    *,
    ...
    rate_limit_general(request)
    return CategoryService.get_category_tasks(
        db=db,
