...
class TaskService:
    ...
    def get_user_tasks(
        ...
        query = db.query(Task).filter(Task.owner_id == user_id).options(selectinload(Task.category))
        ...
        return {"tasks": tasks_response, "total": total, "skip": skip, "limit": limit, "has_more": skip + len(tasks) < total}
    ...
    def _apply_filters(query, filters: TaskFilterParams):
        ...
        if filters.is_overdue is True:
            now = datetime.utcnow()
            query = query.filter(and_(Task.due_date.isnot(None), Task.due_date < now, Task.status.notin_([TaskStatus.DONE, TaskStatus.ARCHIVED])))
    ...
    # ----- CRUD ниже (без изменений бизнес-логики) -----
    @staticmethod
    ...
    def create_task(db: Session, task_create: TaskCreate, user_id: int) -> TaskResponse:
        db_task = Task(**task_create.model_dump(), owner_id=user_id)
    ...
    def update_task(db: Session, task_id: int, task_update: TaskUpdate, user_id: int) -> TaskResponse:
        ...
        if not task: raise NotFoundError(detail="Task not found")
    ...
    def soft_delete_task(db: Session, task_id: int, user_id: int) -> bool:
        task = TaskService.get_task_by_id(db, task_id, user_id)
        if not task: raise NotFoundError(detail="Task not found")
        task.deleted_at = datetime.utcnow(); db.commit(); return True

    @staticmethod
    def restore_task(db: Session, task_id: int, user_id: int) -> TaskResponse:
        task = TaskService.get_task_by_id(db, task_id, user_id)
        if not task: raise NotFoundError(detail="Task not found")
        task.deleted_at = None; db.commit(); db.refresh(task)
        return TaskResponse.from_orm(task)
    ...
    def archive_task(db: Session, task_id: int, user_id: int) -> TaskResponse:
        ...
        task.status = TaskStatus.ARCHIVED; db.commit(); db.refresh(task)
        return TaskResponse.from_orm(task)
    ...
    def get_task_stats(db: Session, user_id: int) -> Dict[str, Any]:
        ...
        now = datetime.utcnow()
        overdue = base.filter(and_(Task.due_date < now, Task.status.notin_([TaskStatus.DONE, TaskStatus.ARCHIVED]))).count()
        today_start = now.replace(hour=0, minute=0, second=0, microsecond=0)
        today_end = now.replace(hour=23, minute=59, second=59, microsecond=999999)
        due_today = base.filter(and_(Task.due_date >= today_start, Task.due_date <= today_end, Task.status.notin_([TaskStatus.DONE, TaskStatus.ARCHIVED]))).count()
        return {"total_tasks": total, "status_counts": status_counts, "priority_counts": priority_counts, "overdue_count": overdue, "due_today_count": due_today}
